...
for i in range(horizon):
    ...
    for t in range(timestep_per_phase): target += [FR_foot0 + offset + \
                                            A*np.sin(2*np.pi*freq * (t+i)*dt + phase)]
    ...
    ocp = optimalControlProblem.OCP(robot=solo, gait=gait, x0=x_mpc[-1], x_ref=x0,\
    ...
    ocp_feet_log = ocp.get_feet_position(x_p)
    ocp_feet_vel_log = ocp.get_feet_velocities(x_p)
    ocp_predictions.append(ocp.get_base_log(x_p))
...
plt.figure(figsize=(12, 6), dpi = 90)
for i in range(3):
    for foot in ocp.terminalModel.contactIds:
        plt.subplot(3,1,i+1)
