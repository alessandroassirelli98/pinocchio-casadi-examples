...
class ShootingNode():
    ...
    def target_cost(self, target):
        ...
        self.cost += casadi.sumsqr(conf.foot_tracking_cost*(self.feet[self.freeIds[0]](self.x) - target)) * self.dt
    ...
    def get_results(self):
        if self.solver == 'ipopt':
            ...
            for t in range(self.T):
                ...
                fsol_to_ws.append(np.concatenate([self.opti.value(self.fs[t][0]), self.opti.value(self.fs[t][1]),  self.opti.value(self.fs[t][2])]))
