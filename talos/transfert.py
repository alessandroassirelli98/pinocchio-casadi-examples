...
class CasadiActionModel:
    ...
    def calc(self,x, u, a, fs, ocp):
        ...
        cost += vcomWeight* casadi.sumsqr( self.vcom(x)- VCOM_TARGET )/2
        ...
        for fid in contactIds:
            ...
            ##DDP## for cid in self.contactIds:
            ##DDP## ocp.subject_to( casadi.sumsqr(self.feet[fid](x)[:2]-self.feet[cid](x)[:2]) >= footMinimalDistance**2)
...
for t in range(T):
    ...
    for i,c in enumerate(runningModels[t].contactIds):
        if t>0 and  c not in runningModels[t-1].contactIds:
            ...
            impactcost=0
            impactcost+=impactAltitudeWeight* casadi.sumsqr(runningModels[t].feet[c](xs[t])[2])/2
            impactcost+=impactVelocityWeight* casadi.sumsqr(runningModels[t].vfeet[c](xs[t]))/2
            impactcost+=impactRotationWeight* casadi.sumsqr(runningModels[t].Rfeet[c](xs[t])[0,2])/2
            impactcost+=impactRotationWeight* casadi.sumsqr(runningModels[t].Rfeet[c](xs[t])[1,2])/2

            # Keep main joints in standard configuration at impact.
            impactcost +=  refMainJointsAtImpactWeight*casadi.sumsqr( xs[t][MAIN_JOINTS] - model.q0[MAIN_JOINTS] )/2

            totalcost += impactcost
            impactCosts.append(impactcost)
...
for t,(refs,r) in enumerate(zip(referenceForces,runningModels)):
    for k,f in enumerate(refs):
        fcost = casadi.sumsqr( (fs[t][6*k:6*k+6] - f)/robotweight )/2 * refForceWeight*DT
        forceCosts.append(fcost)
        totalcost += fcost
...
### Terminal cost
termCost = 0
termCost += terminalNoVelocityWeight*casadi.sumsqr( xs[T][cmodel.nq:] )/2
termCost += terminalXTargetWeight*casadi.sumsqr( terminalModel.base_translation(xs[T])[0] - X_TARGET )/2
totalcost += termCost
...
def save():
    ...
    print(f'Saved "{SOLU_FILE}"!')

    
def xdiff(x0,x):
    return np.concatenate([
        pin.difference(model,x0[:model.nq],x[:model.nq]),
        x[model.nq:]-x0[model.nq:] ])

def evalcost(expr,subs):
    return casadi.Function('tmp',list(subs.keys()),[expr])(*list(subs.values()))

def sol2dict(xgs,ugs,acgs,fgs):
    xdict = { dx: xdiff(x0,xg) for dx,xg in zip(dxs,xgs) }
    udict = { u: ug for u,ug in zip(us,ugs) }
    adict = { ac: acg for ac,acg in zip(acs,acgs) }
    fdict = { f: fg for f,fg in zip(fs,fgs) } 
    return { **xdict, **udict, **adict, **fdict }
    
vsub = sol2dict( xs_sol,us_sol,acs_sol,fs_sol)
